...
- **Rules are managed live via the web UI** (`/dashboard/alert-rules`, admin-only) backed by a dedicated `alert_rules` OpenSearch index (`alertRulesStore.ts`) — create, edit, enable/disable, and delete rules without touching code or redeploying; the worker reads the current rules from the store on every 60s cycle, so changes apply on the next run automatically
...
- **Cooldown / dedup** — `shouldEmit(key, cooldownMinutes)` (`alertDedup.ts`) stores `last_triggered_at` per `(tenant, rule_id, group_value)` in the `alert_state` index and suppresses repeats until the cooldown elapses (default cooldown = `window_minutes`), preventing a continuously-firing condition from re-alerting every tick. It **fails open** (emits) if the state index is unavailable, so alerting never silently stops
...
- Background worker (`alertChecker.ts`) runs every 60s: discovers tenants, calls `evaluateAllRules(tenant)` for each, and persists triggered alerts to the `alerts` OpenSearch index
