export function normalizeApi(data: Record<string, unknown>): Record<string, unknown> {
  const tags = (data.tags as string[]) || [];
  return {
    "@timestamp": (data["@timestamp"] as string) || new Date().toISOString(),
    tenant: (data.tenant as string) || "demoA",
    source: "api",
    vendor: (data.vendor as string) || "api",
    product: (data.product as string) || "api-gateway",
    event_type: (data.event_type as string) || "access",
    event_subtype: (data.event_subtype as string) || "http",
    severity: (data.severity as number) ?? 3,
    action: (data.action as string) || (data.http_method as string) || "",
    ...
    http_method: (data.http_method as string) || "",
